    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          {last && (
            <span className={cn('shrink-0 text-[11px]', unread ? 'text-cyan' : 'text-mist')}>
        ...
        <div className="mt-0.5 flex items-center gap-1">
          {isOwn && last && !last.deleted && (
            last.status === 'READ' ? (
              <CheckCheck className="h-3.5 w-3.5 shrink-0 text-cyan" />
            ) : (
          ...
          {unread && <span className="ml-auto h-2 w-2 shrink-0 rounded-full accent-gradient" />}
        </div>
      </div>
    </Link>
  );
